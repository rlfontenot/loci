...
namespace {
  ...
  std::vector<AMRCellGeometry> source_geometry() {
    return std::vector<AMRCellGeometry>{
      AMRCellGeometry(0,1.0,vector3d<double>(0.0,0.0,0.0)),
      AMRCellGeometry(1,0.25,vector3d<double>(1.0,0.0,0.0)),
      AMRCellGeometry(2,0.75,vector3d<double>(2.0,0.0,0.0)),
      AMRCellGeometry(3,2.0,vector3d<double>(3.0,0.0,0.0))
    } ;
  }

  std::vector<AMRCellGeometry> target_geometry() {
    return std::vector<AMRCellGeometry>{
      AMRCellGeometry(10,0.25,vector3d<double>(-0.75,0.0,0.0)),
      AMRCellGeometry(11,0.75,vector3d<double>(0.25,0.0,0.0)),
      AMRCellGeometry(12,1.0,vector3d<double>(1.75,0.0,0.0)),
      AMRCellGeometry(13,2.0,vector3d<double>(3.0,0.0,0.0))
    } ;
  }

  std::vector<AMRCellContribution> valid_contributions() {
    return std::vector<AMRCellContribution>{
      AMRCellContribution(0,10,0.25,vector3d<double>(-0.75,0.0,0.0)),
      AMRCellContribution(0,11,0.75,vector3d<double>(0.25,0.0,0.0)),
      AMRCellContribution(1,12,0.25,vector3d<double>(1.0,0.0,0.0)),
      AMRCellContribution(2,12,0.75,vector3d<double>(2.0,0.0,0.0)),
      AMRCellContribution(3,13,2.0,vector3d<double>(3.0,0.0,0.0))
    } ;
  }
  ...
  void setup_runtime_mapping(Loci::AMRinterpolation& interpolation) {
    ...
    interpolation.setupRefinementMapping(
      targetSource,sourceVolume,sourceCenterConst,gradientCells,
      gradientDeltas,targetCenterConst,targetVolumeConst,facts) ;
  }
...
TEST_CASE("cell remap plans reject ambiguous or incomplete relations") {
  ...
  SUBCASE("many-to-many relation") {
    const std::vector<AMRCellGeometry> sources{
      AMRCellGeometry(0,1.0,vector3d<double>(0.5,0.0,0.0)),
      AMRCellGeometry(1,1.0,vector3d<double>(1.5,0.0,0.0))
    } ;
    const std::vector<AMRCellGeometry> targets{
      AMRCellGeometry(10,1.0,vector3d<double>(0.5,0.0,0.0)),
      AMRCellGeometry(11,1.0,vector3d<double>(1.5,0.0,0.0))
    } ;
    const std::vector<AMRCellContribution> contributions{
      AMRCellContribution(0,10,0.5,vector3d<double>(0.25,0.0,0.0)),
      AMRCellContribution(0,11,0.5,vector3d<double>(0.75,0.0,0.0)),
      AMRCellContribution(1,10,0.5,vector3d<double>(1.25,0.0,0.0)),
      AMRCellContribution(1,11,0.5,vector3d<double>(1.75,0.0,0.0))
    } ;
    CHECK_FALSE(has_plan(AMRRemapPlan::createCellPlan(
