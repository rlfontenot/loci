...
namespace Loci{
  ...
  if(Loci::MPI_rank == 0) {
    ...
    for(int i=1;i<MPI_processes;++i) {
      ...
      std::vector<vect3d> rv(inner_edge_nodes_sizes[i]) ;
      MPI_Status mstat ;
      MPI_Recv(&rv[0],sizeof(vect3d)*inner_edge_nodes_sizes[i],MPI_BYTE,i,3,MPI_COMM_WORLD,
               &mstat) ;
    ...
    if(num_local_cell_nodes != 0) {
      std::vector<vect3d> v_nodes(num_local_cell_nodes);
      //put inner_nodes in a vector
    ...
    for(int i=1;i<MPI_processes;++i) {
      ...
      int flag = 0 ;
        ...
        std::vector<vect3d> rv(inner_cell_nodes_sizes[i]) ;
        MPI_Status mstat ;
        MPI_Recv(&rv[0],sizeof(vect3d)*inner_cell_nodes_sizes[i],MPI_BYTE,i,5,MPI_COMM_WORLD,
                 &mstat) ;
    ...
    if(num_local_face_nodes != 0) {
      std::vector<vect3d> v_nodes(num_local_face_nodes);
      //put inner_nodes in a vector
    ...
    for(int i=1;i<MPI_processes;++i) {
      ...
      std::vector<vect3d> rv(inner_face_nodes_sizes[i]) ;
      MPI_Status mstat ;
      MPI_Recv(&rv[0],sizeof(vect3d)*inner_face_nodes_sizes[i],MPI_BYTE,i,7,MPI_COMM_WORLD,
               &mstat) ;
    ...
    if(num_local_edge_nodes != 0){
      ...
      MPI_Send(&v_nodes[0],sizeof(vect3d)*num_local_edge_nodes,MPI_BYTE,0,3,MPI_COMM_WORLD) ;
    }
    ...
    if(num_local_cell_nodes != 0){
      ...
      MPI_Send(&v_nodes[0],sizeof(vect3d)*num_local_cell_nodes,MPI_BYTE,0,5,MPI_COMM_WORLD) ;
    }
    ...
    if(num_local_face_nodes != 0){
      ...
      MPI_Send(&v_nodes[0],sizeof(vect3d)*num_local_face_nodes,MPI_BYTE,0,7,MPI_COMM_WORLD) ;
    }
  }
