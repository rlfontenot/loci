...
/** Certain checks in the library slow it down considerably.  Setting
    PreferSpeed to 1 will speed up the library at the expense of
    reversing some tradeoff decisions of the ADF library authors.  */
#ifndef PreferSpeed
#define PreferSpeed	1
#endif
...
/** The ADF format requires a lot of seeking during the creation or deletion
    of nodes.  The Linux 2.4 NFS client is very very slow with fseek/fwrite
    pairs.  Under this regime, node creation can take up to 3 times longer
    than writing the data!  Caching the fwrites allows fewer fseek/fwrite
    commands to be given and speeds up performance under Linux 2.4 by an
    order of magnitude */
#ifndef CACHEWRITES
#if defined(_WIN32)
/** Windows users are most likely going to be writing their file locally,
    so turn cached writes off there. */
#define CACHEWRITES	0
#define kUsePathsForUniqueness	1
#else
#define CACHEWRITES	1
#define kUsePathsForUniqueness	0    /* This should always be zero for Unix */
#endif /* _WIN32 */
#endif
...
#if CACHEWRITES
#include <windows.h>
/** windows.h unfortunately uses NO_ERROR and NO_DATA, which are ADF errors.
    Since changing the name of the ADF errors would break everything that
    uses ADF, we cannot do that.  But if we undefine them, and then redefine
    them, to avoid compiler warnings, then GetLastError() will not work
    (if compared against those two errors).  Since the only reason windows.h
    is included is for the Win32 path functions used in ADFI_open_file(),
    and these do not require the use of GetLastError(), then the best
    solution is to undefine them and write a big warning in the Win32 code. */
#undef NO_ERROR
#undef NO_DATA
#endif /* CACHEDWRITES */
#else
...
***********************************************************************/
/* These were moved from ADF_internals.h since they are only used here
...
static	char   file_version_update[MAXIMUM_FILES][WHAT_STRING_SIZE+1];
   ...
   /** Stores the information required to determine if two FILE* pointers
       are identical.  This is required so that multiple instances of the
       same file can be opened and have the disk caches remain correct.
   **/
struct FILE_INFO {
    dev_t device;
    ino_t inode;
...
static struct FILE_INFO  ADF_file_info[ MAXIMUM_FILES ] ;
...
static struct FILE_PATH_INFO ADF_file_paths[ MAXIMUM_FILES ] = {
...
/* Hack so Windows 64 is treated the same as win32. Don't actually use this
   ...
   the code compares pointer sizes; it never uses them... */
#ifdef _WIN32
#define POINTER_SIZE(_T) 4
#else
#define POINTER_SIZE(_T) sizeof(_T*)
#endif
...
size_t wrapFWrite(void const * ptr, size_t size, size_t nmemb, FILE *stream)
{
  size_t cnt;
  size_t runningCnt = 0;
  size_t maxItemCnt = ((MAX_FWRITE_BUFFERSIZE - 1) / size) + 1;

  while (nmemb > maxItemCnt)
  {
    cnt = fwrite(ptr, size, maxItemCnt, stream);
    if (cnt != maxItemCnt)
      return runningCnt + cnt;

    ptr = ((char const *)ptr) + size * maxItemCnt;
    nmemb -= maxItemCnt;
    runningCnt += maxItemCnt;
  }

  /* nmemb > maxItemCnt implies that the number of members to write
   * here will always be >= 1;
   */
  cnt = fwrite(ptr, size, nmemb, stream);
  return runningCnt + cnt;
}
...
struct DISK_CACHE_NODE {
    ...
    unsigned long block;
    int dirty;
    struct DISK_CACHE_NODE *next;
};

struct DISK_CACHE_NODE *disk_cache = NULL;
struct DISK_CACHE_NODE *disk_cache_end = NULL;

int DC_flush();

/** This function should not be called, and is here so that
    it can be called in the debugger **/
void DC_print()
{
    int count = 0;
    ...
    while (node) {
	printf("   index: %d, block: %ld, dirty: %d\n", node->index, node->block, node->dirty);
	node = node->next;
	count++;
    }
    printf("(Disk cache contains %d blocks)\n", count);
}

/***********************************************************************
DC add block
	Add a file block to the disk cache.  If there is no more space
	in the cache, the cache will be flushed.  The cache is organized
	in a LIFO fashion, in hopes of reducing search costs in
	DC_get_block().  FWRITE_ERRORs that would result from trying to
	write to a read-only file are returned immediately.

input:	unsigned int index		The file to add
input:	unsigned long block		The block to add
		...
		int *error_return )
{
    ...
    int i = 0;
    file_offset_t loc;
    struct DISK_CACHE_NODE *node = disk_cache, *prev = NULL;

    /* If we are writing to a read-only file, generate an error (but still
       add the buffer) */
    if (is_write && ADFI_stridx_c(file_open_mode[ index ], "READ_ONLY" ) == 0) {
	*error_return = FWRITE_ERROR;
	is_write = 0;
    }

    while (i < DISK_CACHE_SIZE - 1 && node &&
	   (!ADFI_is_same_file(node->index, index) || node->block != block)) {
	prev = node;
	node = node->next;
	i++;
    }

    if (!node) {  /* If no node, allocate one */
	node = (struct DISK_CACHE_NODE *)malloc(sizeof(struct DISK_CACHE_NODE));
	if (prev)
	    prev->next = node;
	if (!disk_cache)
	    disk_cache = node;
	node->next = NULL;
	node->dirty = 0;  /* Initialize;  actual set is below */

	/* If this buffer is past the current point, it might actually be
	   a new piece of the file, in which case we'd better write it out */
	loc = file_tell(ADF_file[index]);
	if (is_write && loc <= (file_offset_t)block * (file_offset_t)DISK_BLOCK_SIZE) {
	    if (loc != (file_offset_t)block*(file_offset_t)DISK_BLOCK_SIZE)
	...
	assert(prev || (!prev && node == disk_cache));
	assert(prev != node);

	if (i >= DISK_CACHE_SIZE - 1 && node->dirty)
	    DC_flush();

	if (prev)
	    prev->next = node->next;
	if (node != disk_cache) {
	    node->next = disk_cache;
	    disk_cache = node;
	}
    }
    ...
    node->index = index;
    node->block = block;
    node->dirty = is_write | node->dirty;
    memcpy(node->data, data, DISK_BLOCK_SIZE);
#else
...
/***********************************************************************
DC get block
	Retrieves a file block from the disk cache.  Returns 1 if the
	block is in the cache, 0 otherwise.  If the block is not in
	the cache, the contents of 'data' are undefined.

input:	unsigned int index		The file to get
input:	unsigned long block		The block to get
input:	const char *data		The block's data
		...
		char *data )
{
    ...
    if (node)
	memcpy(data, node->data, DISK_BLOCK_SIZE);
    return(node != NULL);
#else
    return(0);
#endif /* CACHEWRITES */
...
/** Helper function for DC_flush **/
int DC_sort_compare( const void *a, const void *b )
{
    struct DISK_CACHE_NODE *lhs = *(struct DISK_CACHE_NODE **)a;
    struct DISK_CACHE_NODE *rhs = *(struct DISK_CACHE_NODE **)b;
...
/***********************************************************************
DC flush
	Flushes all the dirty cached buffers to disk.

   Possible errors:
FSEEK_ERROR, FWRITE_ERROR
***********************************************************************/
int DC_flush()
{
  int error = NO_ERROR;
  ...
  struct DISK_CACHE_NODE *dirty_buffers[DISK_CACHE_SIZE], *node = disk_cache;

    /* Copy the dirty buffers into an array so that we can sort on
       file position in order to minimize fseek()s (which are really
       slow when combined with fwrite()s and Linux 2.4's NFS client) */
    while (node) {
	if (node->dirty)
	    dirty_buffers[n++] = node;
	node = node->next;
    }

    qsort(dirty_buffers, n, sizeof(struct DISK_CACHE_NODE *), DC_sort_compare);

    for (i = 0;  i < n;  ++i) {
	ADFI_fseek_file(dirty_buffers[i]->index, dirty_buffers[i]->block,
			0, &error);
	if (error == NO_ERROR &&
...
/***********************************************************************
DC clear
	Clears the cache of entries pertaining to the given index.
	Does NOT flush the cache first!
...
None
***********************************************************************/
void DC_clear( unsigned int index )
{
#if CACHEWRITES
  struct DISK_CACHE_NODE *prev = NULL, *node = disk_cache;

  while (node) {
    if (ADFI_is_same_file(node->index, index)) {
      if (node == disk_cache) {
	disk_cache = node->next;
	free(node);
	node = disk_cache;
      }
      else {
...
/***********************************************************************
DC flush range
	Ensures that the specified range of blocks (inclusive) is
	flushed to disk.
...
if( ADF_file[ top_file_index ] != 0 ) {
   ...
   ADFI_flush_caches( top_file_index, error_return );
#if CACHEWRITES
#if kUsePathsForUniqueness
   if (ADF_file_paths[ADF_file_info[top_file_index].device].count <= 1) {
       ADF_file_paths[ADF_file_info[top_file_index].device].count = 0;
       free(ADF_file_paths[ADF_file_info[top_file_index].device].path);
       ADF_file_paths[ADF_file_info[top_file_index].device].path = NULL;
   }
   else
       ADF_file_paths[ADF_file_info[top_file_index].device].count--;
#endif
   ADF_file_info[ top_file_index ].device = 0;
   ADF_file_info[ top_file_index ].inode = 0;
#endif /* CACHEWRITES */
...
/*<<<wro>>>*/
wk_to_os_size = to_os_size;
if (tokenized_data_type &&
    tokenized_data_type[0].type[0] == 'I' &&
    tokenized_data_type[0].type[1] == '8')
  wk_to_os_size = 'B';


switch( EVAL_4_BYTES( from_format, to_format, from_os_size, wk_to_os_size ) ) {
...
void	ADFI_figure_machine_format(
		...
		int *error_return )
{
char	requested_format, requested_os, machine_os_size = 0;
...
if( OK == TRUE ) {
   ...
   if( POINTER_SIZE( float )	!= machine_sizes[i][14] ) OK = FALSE ;
   if( POINTER_SIZE( double )	!= machine_sizes[i][15] ) OK = FALSE ;
   } /* end if */
	 ...
	 int   *error_return )
{
   ...
   *error_return = NO_ERROR ;
   ...
   /*<<<wro>>>*/
   wk_this_machine_os_size = ADF_this_machine_os_size;
   if (tokenized_data_type &&
	   tokenized_data_type[0].type[0] == 'I' &&
	   tokenized_data_type[0].type[1] == '8')
	   wk_this_machine_os_size = 'B';


   if( ADF_file_format[file_index]  == ADF_this_machine_format &&
       ADF_file_os_size[file_index] == wk_this_machine_os_size ) {
      *compare = 1 ;
...
struct	DISK_POINTER		*first_free_block=0, *last_free_block=0;
...
FWRITE_ERROR
***********************************************************************/
void	ADFI_flush_caches(
		const unsigned int file_index,
		int *error_return )
{
    FILE *stream = ADF_file[file_index];

   /* Clear the disk cache.  If there are multiple files open, it
      will clear their disk cache, too, but that's the price of
      simple code... :) */
   clearerr(stream);  /* If we had an error, try clearing it to maybe flush
			 the remaining buffers out */
   *error_return = DC_flush();
   DC_clear( file_index );
}
...
/***********************************************************************
ADFI is same file:
	Returns 1 if the files referenced by the two indices are
	actually the same file.

input:	int index1			File index of first file
input:	int index2			File index of second file
***********************************************************************/
int	ADFI_is_same_file( int index1, int index2 )
{
    /* Invalid indices are always different files (even if they are the
       same invalid index). */
    if ( index1 < 0 || index1 > MAXIMUM_FILES ||
	 index2 < 0 || index2 > MAXIMUM_FILES )
	return(0);

#if CACHEWRITES
    return( ADF_file_info[index1].device == ADF_file_info[index2].device &&
	    ADF_file_info[index1].inode == ADF_file_info[index2].inode );
#else
...
int index ;
FILE *f_ret ;
#if CACHEWRITES && !kUsePathsForUniqueness
struct stat info;
#endif
...
if (last_wr_file >= 0)
{
  // Store last_wr_file since it gets changed in ADFI_flush_buffers.
  int save_file_index = last_wr_file;
  ADFI_flush_buffers(last_wr_file, FLUSH_CLOSE, error_return);
  /* Ignore error from flushing a read-only file that was written to */
  if(ADFI_stridx_c( file_open_mode[save_file_index], "READ_ONLY" ) == 0 &&
     *error_return == FWRITE_ERROR)
    *error_return = NO_ERROR;
}
DC_flush();
...
#if kUsePathsForUniqueness
{
   int i, size, first_free = -1;
   char *path;
#if defined(_WIN32)
{
   /** Note:  since some of the ADF errors conflict with Windows' errors
       (namely NO_ERROR and NO_DATA), comparing the results of GetLastError(),
       should it be used in the future, should not be done against these
       errors. */
   char *short_path;

   size = GetFullPathNameA(file, 0, NULL, NULL);
   short_path = (char *)malloc(size * sizeof(char));  /* Size includes the \0 */
   GetFullPathNameA(file, size, short_path, NULL);
   size = GetLongPathNameA(short_path, NULL, 0);
   path = (char *)malloc((size + 1) * sizeof(char));  /* Size does not include
							 the \0 this time */
   GetLongPathNameA(short_path, path, size);
   free(short_path);
}
#else
/* A Unix implementation is included for completeness.	However, Unix should
   ...
   for (i = 0;	i < MAXIMUM_FILES;  ++i) {
	   ...
	   strcmp(path, ADF_file_paths[i].path) == 0)
	   break;
   }
   if (i < MAXIMUM_FILES) {
       ADF_file_paths[i].count++;
       ADF_file_info[index].device = i;
       free(path);
   }
   else {
       ADF_file_info[index].device = first_free;
       ADF_file_paths[first_free].path = path;
       ADF_file_paths[first_free].count = 1;
   }
   ADF_file_info[index].inode = 0;
}
#else
fstat( fileno( f_ret ), &info ) ;
ADF_file_info[index].device = info.st_dev;
ADF_file_info[index].inode = info.st_ino;
#endif /* kUsePathsForUniqueness */
#endif /* CACHEWRITES */
...
if( ADFI_stridx_c( status, "READ_ONLY" ) == 0 ) {
#if defined(_WIN32) && defined(__WIN32_BINARY__)
   f_ret = _wfopen( file, L"rb" ) ;   /** Open for reading **/
#else
   f_ret = _wfopen( file, L"r" ) ;   /** Open for reading **/
#endif
   } /* end if */
else if( ADFI_stridx_c( status, "OLD" ) == 0 ) {
#if defined(_WIN32) && defined(__WIN32_BINARY__)
   f_ret = _wfopen( file, L"rb+" ) ;   /** Open for both reading & writing **/
#else
   f_ret = _wfopen( file, L"r+" ) ;   /** Open for both reading & writing **/
#endif
   } /* end else if */
else if( ADFI_stridx_c( status, "NEW" ) == 0 ) {
#if defined(_WIN32) && defined(__WIN32_BINARY__)
   f_ret = _wfopen( file, L"wb+" ) ;   /** open new file, or truncate old file */
#else
   f_ret = _wfopen( file, L"w+" ) ;   /** open new file, or truncate old file */
#endif
   } /* end else if */
else if( ADFI_stridx_c( status, "SCRATCH" ) == 0 ) {
   f_ret = tmpfile();
...
#if defined(_WIN32) && defined(__WIN32_BINARY__)
   f_ret = _wfopen( file, L"ab+" ) ;   /** open new, or use existing file **/
#else
   f_ret = _wfopen( file, L"a+" ) ;   /** open new, or use existing file **/
#endif
   } /* end else if */
else {
   *error_return = ADF_FILE_STATUS_NOT_RECOGNIZED ;
...
} /* end of ADFI_open_fileW */

#endif /* USE_WCHAR_FILENAME */
...
if( data_length + block_offset > DISK_BLOCK_SIZE ) {
   ...
   /** We'd better flush the cache before we read, otherwise we will
       get bogus data if our read overlaps with a cached block. **/
   DC_flush_range(file_index, file_block, file_block + (block_offset + data_length) / DISK_BLOCK_SIZE);
   ...
   if( *error_return != NO_ERROR ) {
	...
	/** Read the data from disk **/
   iret = fread( data, 1, data_length, ADF_file[ file_index ] ) ;
...
if( num_in_rd_block < DISK_BLOCK_SIZE ||  /*- buffer is not full -*/
  ...
  if ( (long int) file_block == last_wr_block &&
       ADFI_is_same_file( file_index, last_wr_file ) ) {
    ...
    if ( !DC_get_block( file_index, file_block, rd_block_buffer ) ) {
	...
	/** Read the data from disk **/
	iret = fread( rd_block_buffer, 1, DISK_BLOCK_SIZE, ADF_file[ file_index ] ) ;
	if( (long int)iret == EOF || iret == 0 ) {
	    *error_return = FREAD_ERROR ;

	    return ;
	} /* end if */
	DC_add_block(file_index, file_block, 0, rd_block_buffer, error_return);
    }
    else
	iret = DISK_BLOCK_SIZE;
	...
	/* Clear all entries with current file_index and or type,
	 ...
	 for (i=0; i<MAX_STACK; i++) {
	   if ( stack_mode == INIT_STK ) PRISTK[i].priority_level = -1;
		     ...
		     !ADFI_is_same_file(file_index, PRISTK[i].file_index))
	       continue;
...
if( ( (unsigned long int) data_length + block_offset > DISK_BLOCK_SIZE ||
   ...
   if( *error_return != NO_ERROR ) {
	...
	/** write the buffer **/
   iret=wrapFWrite(wr_block_buffer, 1, DISK_BLOCK_SIZE, ADF_file[last_wr_file]);
#endif
...
if( data_length + block_offset > DISK_BLOCK_SIZE ) {
   ...
   char start_block_data[DISK_BLOCK_SIZE], end_block_data[DISK_BLOCK_SIZE];
   file_offset_t start_pos = ((file_offset_t)file_block) * DISK_BLOCK_SIZE + block_offset;
   file_offset_t end_pos = start_pos + data_length;
   unsigned long ending_block = end_pos / DISK_BLOCK_SIZE;

   /** This originally flushed file_block + 1 to ending_block - 1, but
       if file_block has not been writin to the file yet, the fseek
       will not create it (so the fwrite happens at the current end of the file,
       not in the proper location, and there is the possibility of the file
       being corrupted.  We could fix this by checking to see if block is
       past the end of the file, but always flushing is more robust.*/
   DC_flush_range(file_index, file_block, ending_block);
   ...
   if( *error_return != NO_ERROR ) {
	...
	/** write the data **/
   iret = wrapFWrite( data, 1, data_length, ADF_file[ file_index ] ) ;
   if (iret != data_length)
   {
     *error_return = FWRITE_ERROR ;
     return ;
   ...
   /** Now make sure the cache matches up */
   if (DC_get_block(file_index, file_block, start_block_data)) {
       memcpy(start_block_data + block_offset, data,
	      DISK_BLOCK_SIZE - block_offset);
       DC_add_block(file_index, file_block, 0, start_block_data,
		    error_return);
   }
   if (DC_get_block(file_index, ending_block, end_block_data)) {
       memcpy(end_block_data, data + ending_block * DISK_BLOCK_SIZE - start_pos,
	      end_pos % DISK_BLOCK_SIZE);
       DC_add_block(file_index, ending_block, 0, end_block_data, error_return);
   }
...
if( (long int) file_block != last_wr_block	 ||  /*- a different block -*/
  ...
  if ( (long int) file_block == last_rd_block && (long int) file_index == last_rd_file ) {
    ...
    if ( !DC_get_block( file_index, file_block, wr_block_buffer ) ) {
	/** Position the file **/
	ADFI_fseek_file( file_index, file_block, 0, error_return ) ;
	if( *error_return != NO_ERROR ) {
	    return ;
...
/* For some reason the Sun gets occasial corruptions unless the block is
   cached.  There appears to be no reason in the code for this, and it works
   for other platforms.  It might possibly have something to do with
   disk block sizes on Solaris?  I am removing the else on Solaris, but
