/* ADF write performance on Linux 2.4 NFS is very bad, largely due to
   the fact that ADF does a lot of fseeks.  Writing the modification
   time all the time increases the problem;  about a 10 - 20% performance
   can be achieved by only writing the mod time when the file is closed. */
#ifndef LINUX_NFS_OPT
#define LINUX_NFS_OPT	1
#endif
...
static char ADF_L_identification[] = "@(#)ADF Library Version E01>\001\002\003\004";
...
**/
				 ...
				 /*			       AXXxxx	    */
static char ADF_D_identification[] = "@(#)ADF Database>\001\002\003\004\005\006\007\010A02011>";
				 /*   01234567890123456789012345678901 = 32 */
...
/* This was moved from ADF_internals.h since they are only used here
...
static char link_separator[MAXIMUM_FILES][2] ;
...
CHECK_ADF_ABORT( *error_return ) ;
	...
	/** Write modification time in ADF_Database_Close but nowhere else,
	    otherwise Linux NFS performance is even worse than normal.
	    (Note that if the file was opened read-only this will fail) */
#if LINUX_NFS_OPT
ADFI_write_modification_date( file_index, &tmp ) ;
#endif
...
if( (ADFI_stridx_c( status, "READ_ONLY" ) == 0) ||
   ...
   if( iret != 0 ) { /* File does not exist, this is BAD for OLD */
	...
	/** open the file **/
   ADFI_open_file( filename, status, -1, &file_index, error_return ) ;
   CHECK_ADF_ABORT( *error_return ) ;
   } /* end else if */
   ...
   *error_return = ADF_FILE_STATUS_NOT_RECOGNIZED ;
   CHECK_ADF_ABORT( *error_return ) ;
	...
	/** Initilaize the link separator for the file **/
...
if( (ADFI_stridx_c( status, "READ_ONLY" ) == 0) ||
   ...
   if( iret != 0 ) { /* File does not exist, this is BAD for OLD */
	...
	/** open the file **/
   ADFI_open_fileW( filename, status, -1, &file_index, error_return ) ;
   CHECK_ADF_ABORT( *error_return ) ;
   } /* end else if */
...
if( error_return_input == NO_ERROR ) {
   strcpy( error_string, ADF_error_string[ 0 ] ) ;
	...
	/** Check range of error code **/
	 ...
	 ((unsigned)error_return_input >= sizeof( ADF_error_string )/sizeof(char *) - 1 ) ) {
  snprintf( error_string,80, "ADF: Unrecognized error number %d.",
		error_return_input ) ;
   } /* end else if */
...
lenfilename = strcspn ( link_data, link_separator[file_index] ) ;
...
const size_t link_data_size = ADF_FILENAME_LENGTH +
				  ADF_MAX_LINK_DATA_SIZE + 2 ;
...
if( null_filename == TRUE ) {
  snprintf( link_data, link_data_size, "%s%s", link_separator[file_index], name_in_file ) ;
   } /* end if */
else {
  snprintf( link_data, link_data_size, "%s%s%s", file_name, link_separator[file_index],
	    name_in_file ) ;
   } /* end else */
...
int	      no_data = FALSE ;
double	      LID ;
unsigned long relative_offset = 0, current_chunk = 0, current_chunk_size = 0,
	      past_chunk_sizes = 0;
struct DATA_CHUNK_TABLE_ENTRY	*data_chunk_table = 0;
...
for( i=0; (unsigned)i<total_disk_elements; i++ ) {
   ...
   /** Increment disk pointers, for the special case of one dimensional
      ...
      if( (unsigned)i < total_disk_elements - 1 ) {
