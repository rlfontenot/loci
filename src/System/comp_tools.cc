...
namespace Loci {
  ...
  entitySet process_rule_requests(rule r, fact_db &facts) {
	...
	//cout << d->myid << "    variable  =  "<< *vi << "   tvarmap  =  " << tvarmap[*vi] << endl ;
      }
      const rule_impl::info &rinfo = r.get_info().desc ;
	...
	//cout <<d->myid <<  "   *si  =  "  << *si << "   requests  =  " << requests << endl ;
	entitySet var ;
	...
	if(si->mapping.size() != 0) {
	  for(rmi=si->mapping.rbegin();rmi!=si->mapping.rend();++rmi) {
	    for(rvi=rmi->begin();rvi!=rmi->end();++rvi) {
	      //cout << "map =  " << *rvi << endl ; 
	      temp_requests = facts.preimage(*rvi, temp_requests).first ;
	      temp_requests -= my_entities ;
	    }
	    // cout << d->myid << " before   variable =  " << *vi << " temp requests =   " << temp_requests << endl ;
	  }
	  vector<variableSet>::const_iterator mi ;
	  for(mi=si->mapping.begin();mi!=si->mapping.end();++mi) {
	    for(rvi=mi->begin();rvi!=mi->end();++rvi) 
	      temp_requests = facts.image(*rvi, temp_requests) ;
	  }
	}
	else 
	  temp_requests = EMPTY ;
