...
namespace Loci {
  ...
  variable fact_db::remove_synonym(variable v) const {
  ...
  variable fact_db::get_real_var(variable v) const {
  ...
  void fact_db::create_pure_gfact(const variable& v, gStoreRepP st) {
    //get real variable
  ...
  void fact_db::set_variable_domain_space(const variable& v, gStoreRepP st, gKeySpaceP space){
  ...
  void fact_db::set_variable_image_space(const variable& v, gMapRepP st, gKeySpaceP space){
    if(space != 0){
      //get real variable   
      variable real_v = get_real_var(v) ;
  ...
  void fact_db::remove_variable_domain_space(const variable& v){
  ...
  void fact_db::remove_variable_image_space(const variable& v){
    //get real varaible 
    gStoreRepP st = get_gvariable(v);
  ...
  void fact_db::create_gfact(const variable& v, gStoreRepP st,
  ...
  void fact_db::create_gfact(const std::string& vname, gStoreRepP st,
                              gKeySpaceP domain_space ,
                              gKeySpaceP image_space ) {
  ...
  void fact_db::create_gfact(const variable& v, gstore_instance &si,
                              gKeySpaceP domain_space,
                              gKeySpaceP image_space) {
    variable tmp_v = add_namespace(v) ;
  ...
  void fact_db::create_gfact(const std::string& vname, gstore_instance &si,
                              gKeySpaceP domain_space ,
                              gKeySpaceP image_space ) {
    variable v = variable(vname) ;
  ...
  void fact_db::update_gfact(variable v, gStoreRepP st,
                              gKeySpaceP domain_space ,
                              gKeySpaceP image_space ) {
    //get real variable
    variable tmp_v = add_namespace(v) ;
  ...
  void fact_db::remove_gvariable(variable v) {
  ...
  gStoreRepP fact_db::get_gvariable(variable v) {
    variable tmp_v = add_namespace(v) ;
  ...
  std::istream& fact_db::read_vars(std::istream& s, const rule_db& rdb) {
    ...
    try {
      ...
      for(variableSet::const_iterator vi = working_vars.begin();
        ...
        if(srp==0) std::cerr<< " default variable " << *vi << "is not in fact_db" << endl; 
