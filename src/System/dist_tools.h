...
namespace Loci {
  ...
  void categories(gfact_db &facts,std::vector<entitySet> &pvec) ;
  ...
  void get_mappings(const rule_db &rdb, gfact_db &facts, std::set<std::vector<variableSet> > &maps, unsigned int rule_part = 0) ;
  ...
  std::vector<entitySet> send_entitySetv(const entitySet& e, gfact_db &facts) ;
  ...
  rule_db replace_map_constraints(gfact_db& facts, const rule_db& rdb) ;
}
