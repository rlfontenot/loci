...
DiamondCell* DiamondCell::getSiblingNeib(const Cell* aCell,
  ...
  }else if(parentCell == 0) { // find sibling in aCell
...
DiamondCell* DiamondCell::findNeighbor(const Cell* aCell,
  ...
  if(mf<nfold) {
    return getSiblingNeib(aCell, n2e, mf, nf) ;
  }
  // subroot has no neib
  if(parentCell == 0 && mf >= nfold) {
    // if no face neighbor inside a genaral cell, nf is meaningless
    return 0 ;
  }else { // otherwise find the nearest common ancestor, mf>=nfold
    ...
    while(true) {
      ...
      if((current->parentCell == 0) && (currentFace >= current->nfold)) {
        return 0 ;
      }
    ...
    if(pNeighbor->childCell == 0 || pNeighbor->getLevel() >= getLevel()) {
       return pNeighbor ;
    }
    ...
    // sanity check
    if(this->getLevel() == pNeighbor->getLevel() &&
       this->face[mf] != pNeighbor->face[nf]) {
  ...
  while(!Q.empty()) {
    ...
    switch(currentCode) {
      ...
      case 1:
        ...
        break ;
      default:
        cerr << "WARNING: illegal splitcode in function Cell::reSplit()"
             << endl ;
        break ;
      }
    ...
    for(int i = 0; i < numCells; i++) {
      ...
      for(int ff = 0; ff <numFaces; ff++) {
        ...
        if(!(cells[i]->faceMarked[ff])) {
          ...
          //if no neighbor, don't output the face
          //there is a neighbor
          if(tempNeib != 0) {
            bool isALeaf = tempNeib->childCell == 0 ;
            // if neighbor is a leaf, marked its face
            if(isALeaf) {
              tempNeib ->faceMarked[nf] = true ;
              // if face ff point outward cells[i]
              if(cells[i]->faceOrient[ff] == 0) {
                faces.push_back(make_pair(cells[i]->face[ff],
                                NeibIndex(cells[i]->cellIndex, tempNeib ->cellIndex))) ;
              }else {
                faces.push_back(make_pair(cells[i]->face[ff],
                                NeibIndex( tempNeib ->cellIndex,cells[i]->cellIndex)));
              }
            }
