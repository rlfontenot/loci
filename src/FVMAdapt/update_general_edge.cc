...
class general_tmpedge_points_unit : public unit_rule{
  store<SetLong> pointSet;
public:
  general_tmpedge_points_unit(){
    name_store("tmpPointSet", pointSet);
    output("tmpPointSet");
...
class general_tmpedge_points_apply : public apply_rule<store<SetLong>, SetLongUnion>{
...
public:
  general_tmpedge_points_apply(){
    ...
    input("(tmpFacePlan, is_quadface)");
    input("face2node->pos");
    input("face2edge->edge2node->pos");
    output("face2edge->tmpPointSet");
    input("face2edge->tmpPointSet");
    constraint("faces");
  }
  virtual void compute(const sequence &seq){
...
register_rule<general_tmpedge_points_unit> register_general_tmpedge_points_unit;
register_rule<general_tmpedge_points_apply> register_general_tmpedge_points_apply;

class make_tmpedge_plan:public pointwise_rule{
  const_store<SetLong> pointSet;
  store<std::vector<char> > edgePlan;
public:
  make_tmpedge_plan(){
    name_store("tmpPointSet", pointSet);
    name_store("tmpEdgePlan", edgePlan);
    input("tmpPointSet");
    output("tmpEdgePlan");
...
register_rule<make_tmpedge_plan> register_make_tmpedge_plan;
