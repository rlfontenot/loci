...
class merge_hex_interior_derefineface:public pointwise_rule{
  const_Map cl;
  const_Map cr;
 const_store<char> fl;
  const_store<char> fr;
...
public:
  merge_hex_interior_derefineface(){
    name_store("cl", cl);
    name_store("cr", cr);
    name_store("fl", fl);
    name_store("fr", fr);
...
register_rule<merge_hex_interior_derefineface> register_merge_hex_interior_derefineface;



class merge_hex_boundary_derefineface:public pointwise_rule{
  const_Map cl;
  const_store<char> fl;
   const_store<Array<char,6> > orientCode;
  const_store<std::vector<char> > cellPlan;
  store<std::vector<char> > facePlan;
public:
  merge_hex_boundary_derefineface(){
    name_store("cl", cl);
    name_store("fl", fl);
    name_store("hexOrientCode", orientCode);
    name_store("balancedCellPlan1", cellPlan);
...
register_rule<merge_hex_boundary_derefineface> register_merge_hex_boundary_derefineface;
