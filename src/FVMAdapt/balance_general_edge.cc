...
class general_balancededge_points_unit : public unit_rule{
  store<SetLong> pointSet;
public:
  general_balancededge_points_unit(){
    name_store("balancedPointSet", pointSet);
    output("balancedPointSet");
...
class general_balancededge_points_apply : public apply_rule<store<SetLong>, SetLongUnion>{
...
public:
  general_balancededge_points_apply(){
    ...
    output("face2edge->balancedPointSet");
    input("face2edge->balancedPointSet");
    constraint("faces");
  }
  virtual void compute(const sequence &seq){
...
register_rule<general_balancededge_points_unit> register_general_balancededge_points_unit;
register_rule<general_balancededge_points_apply> register_general_balancededge_points_apply;

class make_balancededge_plan:public pointwise_rule{
  const_store<SetLong> pointSet;
  store<std::vector<char> > edgePlan;
public:
  make_balancededge_plan(){
    name_store("balancedPointSet", pointSet);
    name_store("balancedEdgePlan", edgePlan);
    input("balancedPointSet");
    output("balancedEdgePlan");
...
register_rule<make_balancededge_plan> register_make_balancededge_plan;
