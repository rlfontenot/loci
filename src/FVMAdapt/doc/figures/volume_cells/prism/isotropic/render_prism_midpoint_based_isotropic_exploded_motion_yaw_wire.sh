...
tex_file="$script_dir/prism_midpoint_based_isotropic_exploded.tex"
base_name=prism_midpoint_based_isotropic_exploded_motion_yaw_wire
out_gif="$script_dir/${base_name}.gif"

work_dir=$(mktemp -d "${TMPDIR:-/tmp}/fvmadapt-prism-midpoint-motion-yaw-wire.XXXXXX")
