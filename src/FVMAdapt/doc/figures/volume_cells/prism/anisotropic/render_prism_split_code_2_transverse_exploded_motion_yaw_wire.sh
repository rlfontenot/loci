...
tex_file="$script_dir/prism_split_code_2_transverse_exploded.tex"
base_name=prism_split_code_2_transverse_exploded_motion_yaw_wire
out_gif="$script_dir/${base_name}.gif"

work_dir=$(mktemp -d "${TMPDIR:-/tmp}/fvmadapt-prism-code2-motion-yaw-wire.XXXXXX")
...
render_frame() {
  ...
  pdflatex -halt-on-error -interaction=nonstopmode \
    ...
    "\\def\\fvmadaptExplodeAmount{$amount}\\def\\fvmadaptExplodeScale{1.45}\\def\\fvmadaptYawAngle{$yaw}\\input{$tex_file}" >/dev/null 2>&1
