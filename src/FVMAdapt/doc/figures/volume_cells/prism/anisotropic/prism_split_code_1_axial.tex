...
% Prism split code 1: split axially between the two triangular end faces.
% The split creates one new triangular face and two child prisms.
\begin{document}
\begin{tikzpicture}[
  x={(20.3mm,-8.5mm)},
  y={(15.3mm,11.3mm)},
  ...
  split/.style={draw=splitred,line width=0.52mm},
  ...
  % Midpoints on the three extrusion edges.
  ...
  % The new triangular face is the only interior split surface for code 1.
  \draw[split] (mAD) -- (mBE) -- (mCF) -- cycle;
  ...
  % Edge refinement nodes are red.
  \foreach \p in {mAD,mBE,mCF} {
    \fill[splitred] (\p) circle[radius=0.62mm];
  }
\end{tikzpicture}
\end{document}
