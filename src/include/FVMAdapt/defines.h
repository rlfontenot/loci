...
/**
 * Compare temporary face-node positions by integer `x`/`y` coordinates.
 *
 * This casts only `x` and `y` to `int64`; `z` is ignored. This is used in
 * build_general_cell() with quadface.
 */
...
/**
 * Return the unweighted centroid of the input points.
 *
 * Each point contributes equally; `fnodes` must not be empty.
 */
