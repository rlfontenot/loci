...
#define gradcheck() //if(grad!=0) debugger_() ; //fatal(grad!=0.0) ;
  ...
  FAD2d() : value(0.0), grad(0.0),grad2(0.0) { gradcheck();}
    ...
    /* MPGCOMMENT [05-12-2017 10:35] ---> ADDITION */
    //    template<class T> FAD2d operator +(const T &v) const{
    //      return FAD2d(value+(double)v, grad);
    //    }
    ...
    FAD2d operator +(const long double &v) const{
      return FAD2d(value+(long double)v, grad, grad2);
    }
    ...
    FAD2d operator *(const long double &v) const{
      return FAD2d(value*v,  grad*v, grad2*v);
    }
    ...
    FAD2d operator /(const long double &v) const{
      return FAD2d(value/v, (grad/(long double)v), (grad2/double(v)));
    }
